...
CONFIG = load_config(
    {
        "YANGDO_BLACKBOX_API_HOST": "0.0.0.0",
        "YANGDO_BLACKBOX_API_PORT": "8790",
        "YANGDO_BLACKBOX_ALLOW_ORIGINS": "https://seoulmna.kr,https://www.seoulmna.kr,https://seoulmna.co.kr,https://www.seoulmna.co.kr",
        "YANGDO_BLACKBOX_API_KEY": "",
        "YANGDO_BLACKBOX_ADMIN_API_KEY": "",
        "YANGDO_BLACKBOX_MAX_BODY_BYTES": "65536",
        "YANGDO_BLACKBOX_RATE_LIMIT_PER_MIN": "90",
        "YANGDO_BLACKBOX_TRUST_X_FORWARDED_FOR": "false",
...
logger = setup_logger(name="yangdo_blackbox_api")
...
def _env_bool(key: str, default: bool = False) -> bool:
    raw = os.getenv(key)
    if raw is None or str(raw).strip() == "":
        raw = CONFIG.get(key, "1" if default else "0")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _require_feature(self, feature: str) -> bool:
        ...
        self._send(403, {"ok": False, "error": "tenant_not_allowed"})
        return False

    def _send(self, status: int, payload: Dict[str, Any], extra_headers: Optional[Dict[str, str]] = None) -> None:
    ...
    def _deny_if_blocked_token(self) -> bool:
        ...
        self._send(401, {"ok": False, "error": "blocked_api_key"})
        return True
    ...
    def _require_admin_key(self) -> bool:
        if self._deny_if_blocked_token():
            return False
        expected = self.server.admin_api_keys or self.server.api_keys
    ...
    def do_GET(self):  # noqa: N802
        ...
        path = self.path.split("?", 1)[0]
        if path == "/health":
            self._send(200, {"ok": True, "service": "yangdo_blackbox_api"})
            return
        if path == "/meta":
            if self.server.admin_api_keys and not self._require_admin_key():
                return
    ...
    def do_POST(self):  # noqa: N802
        ...
        if path == "/reload":
            ...
            try:
                ...
                self._send(500, {"ok": False, "error": "reload_failed"})
                self.server.security_events.append(
                    {
                        "event": "reload_failed",
                        "service": "yangdo_blackbox_api",
        ...
        if path == "/estimate":
            if self.server.api_keys and not self._require_api_key():
                return
            if not self._require_feature("estimate"):
                return
            try:
                ...
                self._send(500, {"ok": False, "error": "estimate_failed"})
                self.server.security_events.append(
                    {
...
class BlackboxApiServer(ThreadingHTTPServer):
    def __init__(
        self,
        addr,
        handler_cls,
...
def main() -> int:
    ...
    parser.add_argument("--host", default=_env_str("YANGDO_BLACKBOX_API_HOST", "0.0.0.0"))
    parser.add_argument("--port", type=int, default=_env_int("YANGDO_BLACKBOX_API_PORT", 8790))
    parser.add_argument(
        "--allow-origins",
        default=_env_str(
    ...
    parser.add_argument("--api-key", default=_env_str("YANGDO_BLACKBOX_API_KEY", ""))
    parser.add_argument("--admin-api-key", default=_env_str("YANGDO_BLACKBOX_ADMIN_API_KEY", ""))
    parser.add_argument("--max-body-bytes", type=int, default=_env_int("YANGDO_BLACKBOX_MAX_BODY_BYTES", 65536))
    parser.add_argument("--rate-limit-per-min", type=int, default=_env_int("YANGDO_BLACKBOX_RATE_LIMIT_PER_MIN", 90))
    parser.add_argument("--trust-x-forwarded-for", action="store_true", default=_env_bool("YANGDO_BLACKBOX_TRUST_X_FORWARDED_FOR", False))
    parser.add_argument("--security-log-file", default=_env_str("YANGDO_BLACKBOX_SECURITY_LOG_FILE", "logs/security_blackbox_events.jsonl"))
    ...
    logger.info("[loaded] records=%s train=%s at=%s", meta.get("all_record_count", 0), meta.get("train_count", 0), meta.get("loaded_at"))

    allow_origins = parse_origin_allowlist(str(args.allow_origins or ""))
    api_keys = parse_key_values(str(args.api_key or ""))
    admin_api_keys = parse_key_values(str(args.admin_api_key or ""))
    ...
    logger.info(
        "[security] auth_enabled=%s auth_keys=%s admin_auth=%s admin_keys=%s allow_origins=%s rate_limit_per_min=%s max_body_bytes=%s trust_xff=%s security_log=%s",
        bool(api_keys),
        len(api_keys),
        bool(admin_api_keys),
        len(admin_api_keys),
        ",".join(sorted(allow_origins)) if allow_origins else "(none)",
        int(args.rate_limit_per_min),
        int(args.max_body_bytes),
        bool(args.trust_x_forwarded_for),
        args.security_log_file,
    )
    logger.info(
        "[tenant] enabled=%s strict=%s tenant_count=%s default_tenant=%s config=%s",
        bool(tenant_gateway_enabled),
        bool(tenant_gateway_strict),
        tenant_gateway.tenant_count,
        str(args.tenant_gateway_default_tenant or ""),
        str(args.tenant_gateway_config or ""),
    )
    logger.info("[serving] http://%s:%s", args.host, args.port)
    try:
