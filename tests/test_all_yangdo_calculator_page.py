...
class AllYangdoCalculatorPageTest(unittest.TestCase):
    ...
    def test_calculator_page_html_contains_core_blocks(self):
        ...
        self.assertIn("번호대보다 현재 계산 범위에 더 가까운 매물을 먼저 보여드립니다.", html_customer)
        ...
        self.assertIn("번호대보다 최근 3년 실적·자본금과 현재 범위를 먼저 반영했습니다.", html_customer)
        self.assertIn("표본이 적을 때는 아래 비슷한 사례의 최근 3년 실적과 자본금부터 먼저 보세요.", html_customer)
        self.assertIn("범위가 넓을 때는 아래 추천 매물의 최근 3년 실적 공통대부터 먼저 보세요.", html_customer)
        self.assertIn("1순위 · 3년 실적 기준", html_customer)
        self.assertIn("recommend-actions-title", html_customer)
        self.assertIn("추천 매물 1건이 먼저 보이도록 비교표는 접어두었습니다.", html_customer)
        self.assertIn("추천 매물이 아직 없어 비교표를 숨겨두었습니다. 입력을 조금 보강한 뒤 다시 계산해 보세요.", html_customer)
        ...
        self.assertIn("아래 1순위 보강부터 누르면 해당 입력칸으로 바로 이동합니다.", html_customer)
        self.assertIn("추천 후보가 아직 없습니다. 최근 3년 실적과 자본금을 보강한 뒤 다시 계산해 보세요.", html_customer)
        self.assertIn("추천 후보가 아직 없습니다. 공제조합 잔액을 보강한 뒤 다시 계산해 보세요.", html_customer)
        self.assertIn("추천 후보가 아직 없습니다. 자본금을 보강한 뒤 다시 계산해 보세요.", html_customer)
        self.assertIn("추천 후보가 아직 없습니다. 시평을 조금 더 구체적으로 넣고 다시 계산해 보세요.", html_customer)
        self.assertIn("추천 후보 만들기 2단계", html_customer)
        ...
        self.assertIn("추천 매물 1건의 업종과 가격대를 먼저 비교해 보세요.", html_customer)
