...
def _fallback_min_balance_eok(key, median_balance_eok=None):
    ...
    if isinstance(median_balance_eok, (int, float)) and float(median_balance_eok) > 0:
        return _round4(max(0.16, min(0.6, float(median_balance_eok) * 0.45)))
    return 0.2
    ...
    (function() {{
      ...
      const estimateRemote = async (target) => {{
        ...
        const payload = {{
          ...
          reorg_mode: target.reorg_mode || "",
          sales3_eok: target.sales3_eok,
          sales5_eok: target.sales5_eok,
          balance_eok: target.balance_eok,
          balance_usage_mode: target.balance_usage_mode_requested || target.balance_usage_mode || "",
      ...
      const syncNeighborPanelDisclosure = (force) => {{
        ...
        const shouldOpen = typeof window.matchMedia === "function"
      ...
      const renderRecommendPanelGuide = (out) => {{
        ...
        let text = "";
        if (pubMode === "consult_only" || sourceTier.indexOf("표본 적음") >= 0 || (count > 0 && count <= 3)) {{
          text = "표본이 적을 때는 아래 비슷한 사례 2~3건의 가격대부터 먼저 보세요.";
        }} else if (pubMode === "range_only") {{
          text = "범위가 넓을 때는 아래 추천 매물의 공통 가격대를 먼저 보세요.";
        }}
      ...
      const renderRecommendedListings = (rows, out = null) => {{
        ...
        if (!Array.isArray(rows) || !rows.length) {{
          wrap.innerHTML = `<div class="small">계산 후 입력한 업종·검색축·규모에 가까운 추천 매물이 여기 표시됩니다.</div>`;
          return;
        }}
        const chipPriority = {{
          "업종 일치": 1,
        ...
        wrap.innerHTML = rows.slice(0, 4).map((row) => {{
          ...
          const badge = escapeHtml(compact(row && row.recommendation_label) || "조건 유사");
      ...
      const syncReorgModeRequirement = () => {{
        ...
        if (reorgMode === "분할/합병") {{
          note.textContent = "분할/합병 선택 시 시평·외부신용·부채/유동비율·이익잉여금은 가격 반영에서 제외됩니다.";
          return;
        }}
        ...
        yangdoWizardStepsMeta.forEach((step, stepIndex) => {{
          ...
          document.querySelectorAll(`[data-yangdo-wizard-next="${{stepIndex}}"]`).forEach((button) => {{
            ...
            if (stepIndex === 0) {{
              ...
              nextLabel = "재무·회사 정보";
              disabled = !!state.needsReorg && !state.reorgValue;
            }} else {{
