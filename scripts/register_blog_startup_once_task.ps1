...
# At user logon -> run startup-once blog publish script.
param(
    [string]$RepoRoot = "",
    [string]$TaskName = "SeoulMNA_Blog_StartupOnce",
    [int]$StartupDelaySec = 180
)
...
$runner = Join-Path $RepoRoot "scripts\run_startup_blog_once.ps1"
...
Register-ScheduledTask `
    ...
    -Principal $principal `
    -Settings $settings `
    -Description "Run SeoulMNA blog publish once per day at logon (hidden)." `
    -Force | Out-Null
