...
def build_permit_service_alignment_audit(*, copy_path: Path, rental_path: Path, operations_path: Path, attorney_path: Path) -> Dict[str, Any]:
    ...
    permit_matrix = permit_precheck.get("package_matrix") if isinstance(permit_precheck.get("package_matrix"), dict) else {}

    offerings = rental.get("offerings") if isinstance(rental.get("offerings"), list) else []
    permit_offerings = [row for row in offerings if isinstance(row, dict) and "permit" in _as_list(row.get("systems"))]
    ...
    op_permit_alignment = op_summaries.get("permit_service_alignment") if isinstance(op_summaries.get("permit_service_alignment"), dict) else {}
    ...
    cta_contract_ok = (
        str(((copy_cta.get("primary_self_check") or {}).get("label")) or "") == str(op_permit.get("primary_self_check_cta") or "")
        and str(((copy_cta.get("secondary_consult") or {}).get("label")) or "") == str(op_permit.get("secondary_consult_cta") or "")
        and str(((copy_cta.get("supporting_knowledge") or {}).get("label")) or "") == str(op_permit.get("knowledge_cta") or "")
    )
    if not cta_contract_ok:
        issues.append("cta_contract_mismatch")

    proof_point_contract_ok = (
        int(copy_proof.get("permit_selector_entry_total", 0) or 0) == int(rental_summary.get("permit_selector_entry_total", 0) or 0)
        and int(copy_proof.get("permit_platform_industry_total", 0) or 0) == int(rental_summary.get("permit_platform_industry_total", 0) or 0)
    )
    ...
    service_story_ok = (
        ...
        and bool(copy_summary.get("lane_ladder_ready"))
        and bool(copy_summary.get("service_flow_ready"))
        ...
        and bool(op_permit.get("lane_ladder_ready"))
        and bool(op_permit.get("service_flow_ready"))
    )
    if not service_story_ok:
        issues.append("service_story_not_ready")

    lane_positioning_ok = (
        bool(((permit_matrix.get("summary_self_check") or {}).get("offering_ids")))
        and bool(((permit_matrix.get("detail_checklist") or {}).get("offering_ids")))
        and bool(((permit_matrix.get("manual_review_assist") or {}).get("offering_ids")))
        and bool((copy_lanes.get("summary_self_check") or {}).get("upgrade_target"))
        and bool((copy_lanes.get("detail_checklist") or {}).get("upgrade_target"))
    )
    if not lane_positioning_ok:
        issues.append("lane_positioning_missing")

    rental_positioning_ok = (
        "permit_standard" in widget_standard
        and "permit_pro" in api_or_detail_pro
        and "permit_pro_assist" in api_or_detail_pro
        ...
        and any(isinstance(row, dict) and row.get("offering_id") == "permit_pro_assist" for row in permit_offerings)
    )
    ...
    patent_handoff_ok = (
        ...
        and ("manual-review" in claim_focus or "manual review" in claim_focus or "checklist" in claim_focus)
        and ("api" in commercial_positioning or "manual-review" in commercial_positioning or "self-check" in commercial_positioning or "checklist" in commercial_positioning)
    )
    ...
    return {
        ...
        "summary": {
            ...
            "lane_positioning_ok": lane_positioning_ok,
        ...
        "contracts": {
            "ctas": {
                "primary_self_check": {"copy": str(((copy_cta.get("primary_self_check") or {}).get("label")) or ""), "operations": str(op_permit.get("primary_self_check_cta") or "")},
                "secondary_consult": {"copy": str(((copy_cta.get("secondary_consult") or {}).get("label")) or ""), "operations": str(op_permit.get("secondary_consult_cta") or "")},
                "knowledge": {"copy": str(((copy_cta.get("supporting_knowledge") or {}).get("label")) or ""), "operations": str(op_permit.get("knowledge_cta") or "")},
            },
            "proof_points": {
                "selector_entry_total": {"copy": int(copy_proof.get("permit_selector_entry_total", 0) or 0), "rental": int(rental_summary.get("permit_selector_entry_total", 0) or 0)},
                "platform_industry_total": {"copy": int(copy_proof.get("permit_platform_industry_total", 0) or 0), "rental": int(rental_summary.get("permit_platform_industry_total", 0) or 0)},
            },
            "rental_positioning": {
                ...
                "permit_lane_matrix": {key: _as_list((value or {}).get("offering_ids")) for key, value in permit_matrix.items() if isinstance(value, dict)},
            },
        },
        "attorney_alignment": {
...
def main() -> int:
    ...
    payload = build_permit_service_alignment_audit(copy_path=args.copy, rental_path=args.rental, operations_path=args.operations, attorney_path=args.attorney)
